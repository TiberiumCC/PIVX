...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::TBR);
    ...
    if (!settings.contains("nAnonymizeTiberiumAmount"))
        settings.setValue("nAnonymizeTiberiumAmount", 1000);

    nAnonymizeTiberiumAmount = settings.value("nAnonymizeTiberiumAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeTiberiumAmount"))
        SoftSetArg("-anonymizetiberiumamount", settings.value("nAnonymizeTiberiumAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in tiberium.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeTiberiumAmount:
            return QVariant(nAnonymizeTiberiumAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeTiberiumAmount:
            nAnonymizeTiberiumAmount = value.toInt();
            settings.setValue("nAnonymizeTiberiumAmount", nAnonymizeTiberiumAmount);
            emit anonymizeTiberiumAmountChanged(nAnonymizeTiberiumAmount);
            break;
        case CoinControlFeatures:
