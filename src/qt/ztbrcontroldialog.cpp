...
#include "ui_ztbrcontroldialog.h"
...
std::list<std::string> ZTbrControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZTbrControlDialog::listMints;

ZTbrControlDialog::ZTbrControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZTbrControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZTbrControlDialog::~ZTbrControlDialog()
{
    delete ui;
}

void ZTbrControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZTbrControlDialog::updateList()
{
...
void ZTbrControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZTbrControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZTbr_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZTbrControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZTbrControlDialog::GetSelectedMints()
{
...
void ZTbrControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
