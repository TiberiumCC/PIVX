...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 TBR at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zTBR send
enum ZerocoinSpendStatus {
    ZTBR_SPEND_OKAY = 0,                            // No error
    ZTBR_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZTBR_WALLET_LOCKED = 2,                         // Wallet was locked
    ZTBR_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZTBR_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZTBR_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZTBR_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZTBR_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZTBR_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZTBR_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZTBR_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZTBR_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZTBR_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZTBR_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZTBR_SPENT_USED_ZTBR = 14,                      // Coin has already been spend
    ZTBR_TX_TOO_LARGE = 15                          // The transaction is larger than the max tx size
};
...
public:
    ...
    */
    ...
    void setZTbrAutoBackups(bool fEnabled)
    {
    ...
    /** zTBR reset */
    boost::signals2::signal<void()> NotifyzTBRReset;
};
